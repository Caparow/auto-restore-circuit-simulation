...
class SimulationSystem(
  ...
  def performSimulation(n: Int, p: Int): (Int, Int, List[(BaseElement, Int)]) = {
    dropCounters()
    val generated = generateFailures(n,p)
    generated.foreach(performOneStep)
    ...
    (fails, all, counter.toMap.toList.sortBy(-_._2))
  }
  ...
  private def performOneStep(initState: List[Boolean]): Unit = {
    all += 1
